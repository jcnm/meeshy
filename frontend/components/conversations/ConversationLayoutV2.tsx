...
import { ConversationHeader } from './ConversationHeaderV2';
import { ConversationMessages } from './ConversationMessagesV2';
...
export function ConversationLayout({ selectedConversationId }: ConversationLayoutProps) {
  ...
  const messaging = useMessaging({
    ...
    onTranslation: useCallback((messageId: string, translations: any[]) => {
      console.log('[ConversationLayout] Nouvelles traductions reçues:', { messageId, translations });
      ...
      const newLanguages = translations
        ...
        .filter(lang => lang && !usedLanguages.includes(lang));
  ...
  return (
    <DashboardLayout title={t('conversations.title')} hideHeaderOnMobile={true}>
      <div className={`flex bg-background overflow-hidden conversation-layout ${
        ...
        <main className={cn(
          ...
          {selectedConversation ? (
            <div className="flex flex-col h-full max-h-full">
              ...
              <header className="flex-shrink-0 z-30 bg-card shadow-sm">
                <ConversationHeader
