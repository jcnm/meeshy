...
export function CreateConversationModal({
  ...
  // Identifier availability check states
  ...
  const [identifierCheckMessage, setIdentifierCheckMessage] = useState<string>('');
  ...
  const checkIdentifierAvailability = useCallback(async (identifier: string) => {
    if (!identifier || !identifier.trim() || !validateIdentifier(identifier)) {
    ...
    try {
      const response = await apiService.get<{ success: boolean; available: boolean; identifier: string }>(
        `/api/conversations/check-identifier/${encodeURIComponent(identifier)}`
      );

      if (response.data.success) {
        const available = response.data.available;
        setIdentifierAvailable(available);
        setIdentifierCheckMessage(
          available
      ...
      setIdentifierAvailable(null);
      setIdentifierCheckMessage('Erreur lors de la vérification');
    } finally {
  ...
  // Check identifier availability with debounce
  useEffect(() => {
    // Only check for group and public conversations
    if (conversationType === 'direct' || !customIdentifier || !customIdentifier.trim()) {
      setIdentifierAvailable(null);
      setIdentifierCheckMessage('');
      return;
    }
    ...
    return () => clearTimeout(timer);
  }, [customIdentifier, conversationType, checkIdentifierAvailability]);
            ...
            <div className="space-y-4 p-4 border rounded-lg bg-gradient-to-r from-primary/5 to-secondary/5">
              ...
              <div>
                ...
                <div className="mt-1">
                  ...
                  {/* Identifier availability indicator */}
                  {customIdentifier && validateIdentifier(customIdentifier) && (
                    <div className="mt-2">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
            <Button
              ...
              disabled={
                ...
                (conversationType !== 'direct' && isCheckingIdentifier) ||
                (conversationType !== 'direct' && identifierAvailable === false)
              }
